...
public class Pedidos extends Controller {
	...
	public static void index() {
		...
		try {
			...
			String query = "select * from pedido ORDER BY (cliente)";
			...
			while (rs.next()) {
				...
				Pedido pedido = new Pedido(rs.getString("cliente"),
						rs.getString("pacote"), rs.getString("status"),
						rs.getString("formaPag"), rs.getString("funcionario"),
	...
	public static void inserir() {
		...
		try {
			...
			String query1 = "select * from pacote ORDER BY (nome)";
			...
			while (rs1.next()) {
				...
				Pacote pacote = new Pacote(rs1.getString("nome"),
						rs1.getInt("internet"), rs1.getInt("tv"),
						rs1.getInt("telefone"));
			...
			ResultSet rs2;
			List<Status> retorno2 = new ArrayList<Status>();
		...
		render(clientes,statuses, pacotes, funcionarios);
	}
	...
	public static void editar(String cliente) {
		...
		try {
			...
			while (rs.next()) {
				...
				pedidos.cliente = rs.getString("cliente");
				pedidos.pacote = rs.getString("pacote");
				pedidos.status = rs.getString("status");
				pedidos.formaPag = rs.getString("formaPag");
				pedidos.funcionario = rs.getString("funcionario");
				pedidos.numPedido = rs.getString("numPedido");
				pedidos.dataInscricao = rs.getString("dataInscricao");
				pedidos.dataCadastro = rs.getString("dataCadastro");
		...
		try {
			...
			String query2 = "select * from pacote ORDER BY (nome)";
			pstm2 = (PreparedStatement) con2.prepareStatement(query2);
			...
			while (rs2.next()) {
				...
				Pacote pacote = new Pacote(rs2.getString("nome"),
						rs2.getInt("internet"), rs2.getInt("tv"),
						rs2.getInt("telefone"));
	...
	public static void editar_pedido(String numPedido) {
		...
		try {
			...
			String query1 = "DELETE FROM pedido where numPedido = ?";
			pstm1 = (PreparedStatement) con1.prepareStatement(query1);
			pstm1.setString(1, numPedido);
			pstm1.execute();
